...
class MNISTcnn(object):
	...
	def __init__(self, learning_rate, num_classes, newW1, newW2, newW3, newW4, newW5, newB1, newB2, newB3, newB4, newB5):
		...
		self.old_W5 = tf.placeholder(tf.float32, [N, num_classes-2])
		self.old_B5 = tf.placeholder(tf.float32, [num_classes-2])
		...
		tf.norm(tf.subtract(self.W5[:, :num_classes-2], self.old_W5), ord=1)])
		...
		tf.norm(tf.subtract(self.B5[:num_classes-2], self.old_B5), ord=1)])

		self.overall_weight_diff = tf.add(weight_diff , bias_diff)*10
		...
		self.training_optimizer = tf.train.AdamOptimizer(learning_rate).minimize(self.cost_new)
